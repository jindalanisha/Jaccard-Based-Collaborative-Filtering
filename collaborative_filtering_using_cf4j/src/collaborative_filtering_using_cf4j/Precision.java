...
public class Precision {
	...
	public static void main(String []args)
	{
		...
		PrintableQualityMeasure precision = new cf4j.utils.PrintableQualityMeasure("Precision", numberOfNeighbors, similarityMetrics);
		...
		for (String sm : similarityMetrics) {
			...
			for (int k : numberOfNeighbors) {
				...
				// Compute MAE
				Processor.getInstance().testUsersProcess(new cf4j.qualityMeasures.Precision(1000000,2.5));

				// Retrieve mae an store it
				precision.putError(k, sm,Double.valueOf(new DecimalFormat("#.#####").format( Kernel.gi().getQualityMeasure("Precision"))));
			}

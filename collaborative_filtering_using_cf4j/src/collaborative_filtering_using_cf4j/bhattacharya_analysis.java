...
public class bhattacharya_analysis {
...
//		
		...
		String [] similarityMetrics = {"B", "PCC","cosine","Jaccard","MSD","JMSD","CJMSD"};
		int [] numberOfNeighbors = {40,80,120,160,200,240};
		...
		for (String sm : similarityMetrics) {
			...
			for (int k : numberOfNeighbors) {
				...
				// Compute F1
				Processor.getInstance().testUsersProcess(new cf4j.qualityMeasures.F1(10,4.5));

				// Retrieve F1 an store it
				
				F1.putError(k, sm,Math.sqrt(Kernel.gi().getQualityMeasure("F1")));
